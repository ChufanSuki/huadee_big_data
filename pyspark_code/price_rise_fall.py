...
print("--------------------1.100种货币当日价格----------------------")
df1=df.select('name','symbol','date','open','high','low','close','market_cap').filter(df['date']=='2021-07-11 00:00:00')
...
df2_old=df.select('name','symbol','close').filter(df['date']=='2021-07-10 00:00:00').withColumnRenamed("name","name2").withColumnRenamed("close","close_old")  #昨日收盘价
...
df2=df2.select('name','symbol','close_old','close_new','one_day_%')\
...
df5=df5.select('name','symbol','close_today','close_one_day_ago','one_day_%','close_one_week_ago','one_week_%','close_one_month_ago','one_month_%')  #选取展示属性
